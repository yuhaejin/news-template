...
//            return;
        ...
        if (map != null) {
            ...
            correctName(changes);
            setupDashName(changes, myContext);
...
//                                    sendToArticleQueue(rabbitTemplate,findPk(param),"STOCK",findParam);
                    ...
                    if (handleGiveNTake(change, docNm)) {
                        hasGiveTake = true;
                    }
            ...
            if (isGoodArticle(docNm)) {
                if (hasGiveTake) {
                    sendToArticleQueue(rabbitTemplate, key, "GIVETAKE", key);
                }
            }
...
//            }
    ...
    private void correctName(List<Change> changes) {
        for (Change change : changes) {
            String name = change.getName().trim();
            String _name = modifyStringSpace(name);
            if (name.equalsIgnoreCase(_name)==false) {
                log.info("CORRECT_NAME {} ==> {}", name, _name);
                change.setName(_name);
            }
        }
    }
    private boolean isType12(char c) {
    ...
    private String modifyStringSpace(String s) {
        if (isEmpty(s)) {
            return s;
        }
...
//        return maps;
    ...
    private Map<String, Object> findParentFund(List<Map<String, Object>> tables) {
        for (Map<String, Object> map : tables) {
            log.debug("펀드정보 " +map.toString());
            if (containsKey(map,"보고자구분") && containsKey(map,"직업(사업내용)")) {
...
//    }
     ...
     * @return
     */
    private boolean findCompressedData(List<Change> changes, String rptNm, String docUrl, String contents) throws Exception {
...
//                String now = strings[1];
        ...
        }else if(rptNm.contains("최대주주등소유주식변동신고서")){

            if (changes.size() == 1) {
                Change change = changes.get(0);
                Map<String, Object> map = findShareHolderTable(contents, docUrl);
                if (map == null) {
                    log.debug("[압축STOCKDATA] 분석할 테이블이 없음.... ");
                    return false;
                }
                String[] strings = findBeforeNowDate(map);
                String before = strings[0];
                // 공시의 이번보고서 작성기준일은 사용하지 않음. 현재 처리하는게 이미 DB에 존재할 수 있기 때문에..
//                String now = strings[1];
                if (StringUtils.isAnyEmpty(before)) {
            ...
            log.debug("[압축STOCKDATA] 압축거래 관련 공시가 아님. ");
            return false;
        }
    }
    ...
    private Map<String,Object> findShareHolderTable(String contents, String docUrl){
        try {
            StockChangeSelectorByPattern2 pattern2 = new StockChangeSelectorByPattern2();
            pattern2.setKeywords(new String[]{"보고", "개요"});
            TableParser stockChangeTableParser = new TableParser(pattern2);
            ReportDateParser myParser = new ReportDateParser();
            return (Map<String,Object>)stockChangeTableParser.simpleParse(contents, docUrl,myParser);
        } catch (Exception e) {
    ...
    /**
     * 최대주주 공시 보고의 개요에서 직접보고서제출일, 이번보고서제출일 추출
     * @param map
     * @return
     */
    private String[] findBeforeNowDate(Map<String, Object> map) {
        String[] strings = new String[2];
        strings[0] = findBeforeDate(map);
        strings[1] = findNowDate(map);
     ...
     * @return
     */
    private String findBeforeDate(Map<String,Object> map) {
        return findDate(map, "직전보고서");
    }

    private String findNowDate(List<Map<String, Object>> maps) {
        return findDate(maps, "이번보고서");
    }
    private String findNowDate(Map<String,Object> map) {
        return findDate(map, "이번보고서");
    }
    private String findDate(Map<String, Object> map, String type) {
     ...
     * @param change
     */
    private boolean handleGiveNTake(Change change, String docNm) {
