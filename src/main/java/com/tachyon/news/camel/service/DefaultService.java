...
public abstract class DefaultService {
    ...
    public DefaultService(TemplateMapper templateMapper) {
...
//    protected void view(Exchange exchange, Logger log) {
//        Map<String,Object> properties = exchange.getProperties();
//        log.debug("exchage properties ...");
//        view(properties);
//        log.debug("exchage in headers ...");
//        view(exchange.getIn().getHeaders());
//    }
    protected void infoView(Exchange exchange, Logger log) {
//        viewByInfo(exchange, log);
        viewByDebug(exchange, log);
    }

    private void viewByInfo(Exchange exchange, Logger log) {
        Map<String,Object> properties = exchange.getProperties();
        log.info("exchage properties ...");
        infoView(properties,log);
        log.info("exchage in headers ...");
        infoView(exchange.getIn().getHeaders(),log);
    }
    private void viewByDebug(Exchange exchange, Logger log) {
        Map<String,Object> properties = exchange.getProperties();
        log.debug("exchage properties ...");
        debugView(properties,log);
        log.debug("exchage in headers ...");
        debugView(exchange.getIn().getHeaders(),log);
    }

    protected void requestBodyAndHeaders(CamelContext camelContext,String uri,String body, Map<String,Object> headers) {
        camelContext.createProducerTemplate().sendBodyAndHeaders(uri, ExchangePattern.InOnly, body,headers );
    }
    ...
    protected void requestBodyAndHeaders(RabbitTemplate rabbitTemplate, String uri, Object body) {
        rabbitTemplate.convertAndSend(uri, body);
    }
     ...
     * @param headers
     */
    protected void setupPreviousHeaders(Exchange exchange, Map<String, Object> headers) {
        Map<String, Object> in = exchange.getIn().getHeaders();
    ...
    protected void handleError(RabbitTemplate rabbitTemplate, Exchange exchange, Exception e, Logger logger) {
        String key = findKey(exchange);
        String fromQueue = findRoutingKey(exchange);
    ...
    protected String findKey(Exchange exchange) {
        byte[] bytes= (byte[])exchange.getIn().getBody();
        return new String(bytes, Charset.forName("UTF-8"));
    }

    protected String findRoutingKey(Exchange exchange) {
        return findHeader(exchange, "rabbitmq.ROUTING_KEY");
    }

    protected String findHeader(Exchange exchange,String key) {
