...
public class IndexingCommand extends BasicCommand {
    ...
    public void execute(Exchange exchange) throws Exception {
        ...
        boolean reindexing = doReIndexing(exchange,key);
        ...
        Map<String, Object> map = findKongsiHalder(myContext, exchange, templateMapper, docNo, code, acptNo);
        if (map == null) {
    ...
    private boolean doReIndexing(Exchange exchange,String key) {
        if (exchange.getIn().getHeaders().containsKey(NewsConstants.REINDEXING)) {
            return true;
        } else {
